...
const TemplateSelector = ({ selectedTemplate, onChange }) => {
  ...
  const templates = [
    ...
    },
    {
      ...
      name: "Modern",
      preview:
        "A sleek, contemporary resume design with bold typography,clean layout and modern font choices",
    },
    {
      id: "minimal-image",
      name: "Minimal-Image",
        ...
        <div className="absolute top-full w-xs p-3  mt-2 space-y-3 z-10  bg-white rounded-md border border-gray-200 shadow-sm">
          {templates.map((template) => (
            <div key={template.id} onClick={()=> {onChange(template.id);
                setIsOpen(false)}} className={`relative p-3 border roounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? 
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="mt-2 p-2 bg-blue-50 rounded text text-xs text-gray-500 italic">{template.preview}</div>
                    </div>
            </div>
          ))}
        </div>
